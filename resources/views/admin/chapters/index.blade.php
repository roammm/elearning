...
<head>
    ...
    <style>
        ...
        .card-desc{color:#6b7280;font-size:14px;margin-bottom:16px}
...
<body>
    ...
    <div class="container" style="padding-top:100px">
        <div class="page-header">
            <div>
                <h1 class="page-title">Manage Chapters - {{ $course->title }}</h1>
                <p style="color:#6b7280">Kelola chapters untuk course ini</p>
            </div>
            <a href="{{ route('admin.chapters.create', $course) }}" class="btn">+ Tambah Chapter</a>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-desc">{{ $chapter->description }}</div>
            <div class="card-meta">Order: {{ $chapter->order }} | Parts: {{ $chapter->parts_count ?? 0 }}</div>
            <div class="card-actions">
                <a href="{{ route('admin.parts', $chapter) }}" class="btn btn-secondary">Manage Parts</a>
                <a href="{{ route('admin.chapters.edit', $chapter) }}" class="btn">Edit</a>
                <form action="{{ route('admin.chapters.destroy', $chapter) }}" method="POST" style="display:inline" onsubmit="return confirm('Yakin ingin menghapus chapter ini?')">
                    @csrf
                    @method('DELETE')
        ...
        @empty
        <div class="card">
            <p style="color:#6b7280;text-align:center;padding:40px">Belum ada chapter. <a href="{{ route('admin.chapters.create', $course) }}" style="color:#2563eb">Tambah chapter pertama</a></p>
        </div>
        @endforelse
