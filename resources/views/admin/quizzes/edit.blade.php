        ...
        <div class="card">
            <form action="{{ route('admin.quizzes.update', $quiz) }}" method="POST" id="quiz-form">
                ...
                <div class="form-group">
                    ...
                    <textarea name="question" class="form-textarea" required>{{ old('question', $quiz->question) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <div id="options-container">
                        @foreach(old('options', $quiz->options) as $option)
                        <div class="option-input">
                            <input type="text" name="options[]" class="form-input" value="{{ $option }}" required>
                            <button type="button" onclick="removeOption(this)" class="btn btn-sm" style="background:#ef4444">Hapus</button>
                        </div>
                        @endforeach
                    </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="answer" class="form-input" value="{{ old('answer', $quiz->answer) }}" required placeholder="Harus sama persis dengan salah satu option">
                    @error('answer')<div class="error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label class="form-label">Order *</label>
                    <input type="number" name="order" class="form-input" value="{{ old('order', $quiz->order) }}" min="0" required>
                    @error('order')<div class="error">{{ $message }}</div>@enderror
                </div>

                <div style="display:flex;gap:12px;margin-top:32px">
                    <button type="submit" class="btn">Update</button>
                    <a href="{{ route('admin.quizzes', $quiz->course) }}" class="btn btn-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
