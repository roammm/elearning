...
<body>
    ...
    <div class="container" style="padding-top:100px">
        <div class="page-header">
            <div>
                <h1 class="page-title">Manage Parts - {{ $chapter->title }}</h1>
                <p style="color:#6b7280">Kelola parts untuk chapter ini</p>
            </div>
            <a href="{{ route('admin.parts.create', $chapter) }}" class="btn">+ Tambah Part</a>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-meta">Order: {{ $part->order }}</div>
            <div class="card-actions">
                <a href="{{ route('admin.parts.edit', $part) }}" class="btn">Edit</a>
                <form action="{{ route('admin.parts.destroy', $part) }}" method="POST" style="display:inline" onsubmit="return confirm('Yakin ingin menghapus part ini?')">
                    @csrf
                    @method('DELETE')
        ...
        @empty
        <div class="card">
            <p style="color:#6b7280;text-align:center;padding:40px">Belum ada part. <a href="{{ route('admin.parts.create', $chapter) }}" style="color:#2563eb">Tambah part pertama</a></p>
        </div>
        @endforelse

        <div style="margin-top:32px">
            <a href="{{ route('admin.chapters', $chapter->course) }}" class="btn btn-secondary">← Kembali ke Chapters</a>
        </div>
    </div>
</body>
</html>
